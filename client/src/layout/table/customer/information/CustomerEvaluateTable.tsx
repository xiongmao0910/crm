...
const CustomerEvaluateTable = ({ idCustomer }: TInfoCustomerTableProps) => {
    ...
    const { isError, isFetching, isLoading, refetch } = useQuery({
        ...
        onSuccess: (data: TResponsePageDto<TCustomerEvaluateDto[]>) => {
            ...
            <Box sx={{ display: "flex", gap: "0.25rem" }}>
                ...
                <Tooltip arrow placement="right" title="Yêu cầu xoá">
                    <IconButton
                        color="error"
                        onClick={() => {
                            const { id } = row.original as TCustomerEvaluateDto;
                            setIdSelected(id);
                            openDeleteModal();
                        }}
                    >
